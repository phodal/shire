...
class UserCustomVariableResolver(
    ...
    override suspend fun resolve(initVariables: Map<String, Any>): Map<String, String> {
        ...
        return context.hole?.variables?.mapValues {
            val value = PatternActionProcessor(context.myProject, context.hole, vars).execute(it.value)
            val variables = vars + (it.key to value)
            record.addSnapshot(variables, this)
            value
        } ?: emptyMap()
    }
}